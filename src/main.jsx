...
function App() {
  ...
  // --- CARGA DE DATOS ---
  useEffect(() => {
    const saved = localStorage.getItem('maac_data_consolidated');
    if (saved) {
  ...
  // --- GUARDADO DE DATOS ---
  useEffect(() => {
    localStorage.setItem('maac_data_consolidated', JSON.stringify({ metadata, weights, assessments }));
  ...
  const calculation = useMemo(() => {
    ...
    const factorDetails = FACTORS.map(f => {
      let sumQuality = 0;
      let countChars = f.characteristics.length;

      f.characteristics.forEach(c => {
        const ass = assessments[c.id] || { level: 0 };
        sumQuality += Number(ass.level);
      });

      // Promedio de calidad del factor (0 a 4)
      const avgFactorQuality = sumQuality / countChars;
      
      // Peso del factor (0 a 100)
      const fWeight = Number(weights.factors[f.id] || 0);
      
      // Puntaje por Factor = Calidad * (Peso / 100)
      const scoreInFactor = avgFactorQuality * (fWeight / 100);
      ...
      return { 
        id: f.id, 
        title: f.title, 
        avgQuality: avgFactorQuality, 
        scoreInFactor: scoreInFactor, 
        weight: fWeight 
      };
    });
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden text-sm">
      <aside className="w-80 bg-slate-900 text-white flex flex-col p-6 space-y-4 shadow-2xl">
        ...
        <nav className="flex-1 overflow-y-auto space-y-2">
          <button onClick={()=>setView('setup')} className={`w-full p-4 rounded-2xl flex items-center font-bold ${view === 'setup' ? 'bg-blue-600' : 'text-slate-400'}`}><Settings className="mr-3" size={18}/> 1. Configuración</button>
          <button onClick={()=>setView('assessment')} className={`w-full p-4 rounded-2xl flex items-center font-bold ${view === 'assessment' ? 'bg-blue-600' : 'text-slate-400'}`}><FileText className="mr-3" size={18}/> 2. Evaluación</button>
          <button onClick={()=>setView('dashboard')} className={`w-full p-4 rounded-2xl flex items-center font-bold ${view === 'dashboard' ? 'bg-blue-600' : 'text-slate-400'}`}><BarChart2 className="mr-3" size={18}/> 3. Reporte Final</button>
          <div className="h-px bg-slate-800 my-4" />
          {FACTORS.map((f, i) => (
            <button key={f.id} onClick={()=>{setActiveFactorIndex(i); setActiveCharIndex(0); setView('assessment');}} className={`w-full text-left p-3 rounded-xl text-[10px] font-black ${activeFactorIndex === i && view === 'assessment' ? 'text-blue-400 bg-slate-800' : 'text-slate-500'}`}>FACTOR {f.id}</button>
          ))}
        </nav>
        <button onClick={()=>{localStorage.clear(); window.location.reload();}} className="p-4 text-red-500 text-[10px] font-black uppercase hover:bg-red-500/10 rounded-xl">Reiniciar</button>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {view === 'setup' && (
          <div className="flex-1 overflow-y-auto p-12 bg-white space-y-12">
            ...
            <section className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-200 grid grid-cols-2 gap-8 shadow-inner text-slate-600 font-bold">
               <div className="space-y-4">
                  <input type="text" placeholder="Nombre Programa" className="w-full p-4 rounded-2xl border-none shadow-sm" value={metadata.name} onChange={e=>setMetadata({...metadata, name: e.target.value})} />
                  ...
                  <input type="date" className="w-full p-4 rounded-2xl border-none shadow-sm" value={metadata.regDate} onChange={e=>setMetadata({...metadata, regDate: e.target.value})} />
               </div>
               <div className="space-y-4">
                  <input type="date" className="w-full p-4 rounded-2xl border-none shadow-sm" value={metadata.committeeDate} onChange={e=>setMetadata({...metadata, committeeDate: e.target.value})} />
                  <input type="date" className="w-full p-4 rounded-2xl border-none shadow-sm" value={metadata.assessmentDate} onChange={e=>setMetadata({...metadata, assessmentDate: e.target.value})} />
               </div>
            </section>
            <section className="space-y-8">
               <div className="flex justify-between items-center"><h3 className="text-2xl font-black">Asignación de Pesos (%)</h3><div className={`px-6 py-2 rounded-full font-black text-xs ${factorSum === 100 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>Suma Factores: {factorSum}%</div></div>
               <div className="grid grid-cols-1 gap-4">
                  {FACTORS.map(f => (
                    <div key={f.id} className="bg-slate-50 p-6 rounded-3xl flex justify-between items-center border">
                      <span className="font-black uppercase text-slate-500">Factor {f.id}: {f.title}</span>
                      <div className="flex items-center bg-white px-4 py-2 rounded-xl border"><input type="number" className="w-16 text-center font-black border-none" value={weights.factors[f.id] || ""} onChange={e=>setWeights({...weights, factors: {...weights.factors, [f.id]: e.target.value}})} /><span className="ml-2 font-bold text-slate-300">%</span></div>
                    </div>
                  ))}
               </div>
            </section>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-12 bg-slate-50 space-y-8">
              <div className="max-w-4xl mx-auto space-y-8">
                ...
                <textarea className="w-full h-48 p-8 bg-white rounded-[2.5rem] border outline-none text-sm font-medium text-slate-600" value={currentAss.observations} onChange={e=>setAssessments({...assessments, [activeC.id]: {...currentAss, observations: e.target.value}})} placeholder="Escriba hallazgos..." />
              </div>
            </div>
        ...
        {view === 'dashboard' && (
          <div className="flex-1 overflow-y-auto p-12 bg-slate-50 space-y-10">
            <header className="flex justify-between items-end border-b pb-8">
              <div><h2 className="text-4xl font-black text-slate-800 tracking-tighter uppercase">Reporte de Resultados</h2><p className="text-slate-400 font-bold uppercase text-[11px] mt-2 tracking-widest">{metadata.name} | Sede {metadata.campus}</p></div>
              <button onClick={() => window.print()} className="bg-green-600 text-white px-8 py-4 rounded-3xl font-black text-[10px] tracking-widest uppercase shadow-xl hover:bg-green-700">Imprimir PDF</button>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs font-bold text-slate-300 mt-2 italic">Sobre 4.00 máximos</p>
                </div>
                <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white flex flex-col justify-center">
                    <div className="flex items-center mb-4"><Award className="text-blue-400 mr-3" size={24}/><span className="font-bold uppercase tracking-widest">Estado Calidad</span></div>
                    <p className="text-2xl font-black italic">{calculation.globalScore >= 3.6 ? "ALTA CALIDAD EXCELENTE" : calculation.globalScore >= 3.0 ? "ALTA CALIDAD CUMPLIDA" : "BAJA CALIDAD / MEJORA"}</p>
                </div>
            </div>

            <div className="bg-white p-10 rounded-[3rem] shadow-sm border space-y-4">
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight mb-4">Puntaje por Factor Académico</h3>
                <div className="space-y-4">
                    {calculation.factorDetails.map(f => (
                        <div key={f.id} className="p-6 bg-slate-50 rounded-3xl flex justify-between items-center border hover:bg-white hover:shadow-lg transition-all group">
                            ...
                            <div className="flex gap-10 items-center">
                                <div className="text-center"><p className="text-[8px] font-black text-slate-400 uppercase">Calidad (0-4)</p><p className="font-bold text-slate-600">{f.avgQuality.toFixed(2)}</p></div>
                                ...
                                <div className="text-right min-w-[140px] border-l pl-10"><p className="text-[8px] font-black text-blue-400 uppercase">Puntaje por Factor</p><p className="font-black text-blue-600 text-xl">{f.scoreInFactor.toFixed(2)}</p></div>
                            </div>
                        </div>
                    ))}
                </div>
