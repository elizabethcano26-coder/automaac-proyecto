...
const callGeminiAPI = async (prompt) => {
  const apiKey = ""; // Pega tu clave aquí si deseas usar la IA
  try {
  ...
  return (
    <div className="p-10 max-w-5xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-black text-slate-800 tracking-tighter uppercase">Tablero MAAC 2025</h2>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
        <h3 className="font-bold text-slate-400 uppercase text-xs tracking-widest flex items-center"><FolderOpen size={16} className="mr-2"/> Configuración del Programa</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input type="text" placeholder="URL Carpeta Raíz" className="p-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-medium text-blue-600" value={programConfig.rootFolder} onChange={e => setProgramConfig({...programConfig, rootFolder: e.target.value})} />
        </div>
      </div>
        ...
        {scores.map(f => (
          <div key={f.id} onClick={() => onNavigate(f.id-1)} className="bg-white p-6 rounded-3xl border border-slate-100 hover:border-blue-200 cursor-pointer transition-all group">
            <div className="flex justify-between items-center mb-4">
              <span className="font-bold text-slate-700 group-hover:text-blue-600 transition-colors uppercase">FACTOR {f.id}: {f.title}</span>
...
function App() {
  ...
  const handleReset = () => {
    if (window.confirm("¿Deseas borrar todo el progreso?")) { setAssessments({}); setProgramConfig({name:"", rootFolder:""}); localStorage.clear(); }
  };

  const handleAI = async () => {
    setAiLoading(true);
    const text = await callGeminiAPI(`Como experto MAAC, sugiere evidencias para "${activeC.title}" en nivel ${current.level}.`);
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans">
      {aiContent && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6"><div className="bg-white rounded-[2rem] p-10 max-w-2xl w-full max-h-[80vh] overflow-y-auto relative"><button onClick={()=>setAiContent(null)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-900"><X/></button><h3 className="text-2xl font-black mb-6 flex items-center text-blue-600"><Lightbulb className="mr-2"/> Guía IA</h3><div className="prose prose-slate text-sm leading-relaxed">{aiContent}</div></div></div>}
      <aside className="w-80 bg-white border-r flex flex-col p-6 space-y-8 shadow-xl">
        <div className="py-4 font-black text-2xl text-blue-600 tracking-tighter italic">AUTO-MAAC</div>
        <nav className="flex-1 space-y-2 overflow-y-auto">
          <button onClick={()=>setView('dashboard')} className={`w-full p-4 rounded-2xl flex items-center font-bold text-sm ${view === 'dashboard' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><BarChart2 size={18} className="mr-3"/> Dashboard</button>
          <div className="h-px bg-slate-100 my-4" />
          {FACTORS.map((f, i) => (
            <button key={f.id} onClick={()=>{setActiveFactorIndex(i); setActiveCharIndex(0); setView('assessment');}} className={`w-full text-left p-4 rounded-2xl text-[10px] font-black transition-all ${activeFactorIndex === i && view === 'assessment' ? 'bg-slate-900 text-white' : 'text-slate-400 hover:bg-slate-50 uppercase'}`}>FACTOR {f.id}</button>
          ))}
        </nav>
        ...
        {view === 'assessment' ? (
          <>
            <header className="p-12 bg-white border-b flex justify-between items-end">
              <div><span className="text-blue-600 font-black text-xs tracking-widest block mb-2 uppercase tracking-tighter">Característica {activeC.id}</span><h2 className="text-3xl font-black text-slate-800 tracking-tight">{activeC.title}</h2></div>
            ...
            <div className="flex-1 overflow-y-auto p-12 bg-slate-50 space-y-8">
              <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center"><LinkIcon size={14} className="mr-2"/> Link Carpeta Evidencias</h3>
                  <input type="text" className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none focus:ring-2 focus:ring-blue-600 text-blue-600 font-medium" value={current.evidenceLink} onChange={e=>setAssessments({...assessments, [activeC.id]: {...current, evidenceLink:e.target.value}})} placeholder="Pegue el enlace aquí..." />
                </div>
                <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
                  ...
                  <textarea className="w-full h-64 bg-slate-50 border-none rounded-3xl p-6 outline-none focus:ring-2 focus:ring-blue-600 font-medium text-slate-600" value={current.observations} onChange={e=>setAssessments({...assessments, [activeC.id]: {...current, observations:e.target.value}})} placeholder="Escriba aquí..." />
                </div>
              </div>
            </div>
            <footer className="p-8 bg-white border-t flex justify-between px-12">
              <button onClick={()=>setActiveCharIndex(Math.max(0, activeCharIndex-1))} className="font-black text-slate-400 hover:text-slate-800 transition-all flex items-center tracking-widest text-[10px]">ANTERIOR</button>
              <button onClick={()=>{ if(activeCharIndex < activeF.characteristics.length-1) setActiveCharIndex(activeCharIndex+1); else if(activeFactorIndex < FACTORS.length-1) { setActiveFactorIndex(activeFactorIndex+1); setActiveCharIndex(0); } else setView('dashboard'); }} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:bg-blue-700 transition-all tracking-widest text-[10px]">SIGUIENTE</button>
            </footer>
          </>
        ) : (
